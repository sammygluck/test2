		...
		async (request, reply) => {
			try {
				const result = await fastify.sqlite.get(
					"SELECT users.id, users.username, users.email, users.created_at, users.updated_at, users.blocked_users, users.friends, users.avatar, users.full_name, users.alias, users.two_factor_auth FROM users WHERE id = ?",
					[request.params.id]
				);
				if (!result) {
